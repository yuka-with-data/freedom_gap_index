...
## Population against 3 Life Variables
...
fi_gap <- free_gap %>% 
  select(CL, PR, is_ldc) # extract 3 variables
# View(fi_gap)
```
...
free_matrix <- free_gap %>%
  select(-c(country, year, Status, continent, region, dol_per_day))
# View(free_matrix)
```
...
trg_model <- multinom(training.Status ~ ., data = trg_pred)
summary(trg_model)

# trg_model <- gam(training.Status ~ .,
#                  family = gaussian,
#                  data = trg_pred)
```
...
```{r}
multinom_mod <- multinom(Status ~ ., data = training)
```
...
dpd_df <- free_gap %>%
  select(-c(country, year, continent, region, gdp_per_cap))
summary(dpd_df)
```
